...
bytes32 constant TYPE_ONE_TYPEHASH = keccak256(
    "TypeOne(uint256 operation,address contractAddress,uint256[] rules,bool flag,uint8[] approversGiven,address[] logicsList,bytes constantsList,AStruct structure1,BStruct structure2,TypeTwo structure3,TypeTwo structure4,TypeTwo structure5,TypeTwo structure6,bytes32 nonce,string operationId)AStruct(bool exists)BStruct(string custom)TypeTwo(string tests,uint8 testEnum)"
);

bytes32 constant TYPE_TWO_TYPEHASH = keccak256(
    "TypeTwo(string tests,uint8 testEnum)"
);
