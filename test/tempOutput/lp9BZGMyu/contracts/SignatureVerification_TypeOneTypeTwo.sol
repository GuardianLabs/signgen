...
contract SignatureVerification_TypeOne_TypeTwo {
        ...
        return
            abi.encode(
                ...
                keccak256(
                    abi.encode(
                        TYPE_TWO_TYPEHASH,
                        keccak256(bytes(message.structure4.tests)),
                        uint8(message.structure4.testEnum)
                    )
                ),
                keccak256(
                    abi.encode(
                        TYPE_TWO_TYPEHASH,
                        keccak256(bytes(message.structure5.tests)),
                        uint8(message.structure5.testEnum)
                    )
                ),
                keccak256(
                    abi.encode(
                        TYPE_TWO_TYPEHASH,
                        keccak256(bytes(message.structure6.tests)),
                        uint8(message.structure6.testEnum)
                    )
                ),
                message.nonce,
    ...
    function recoverTypeTwo(
        ...
        bytes32 structHash = keccak256(encodeTypeTwoParameters(message));
    ...
    function verifyTypeTwo(
        ...
        require(
            recoverTypeTwo(message, signature, domainSeparator) == addr,
            errMessage
        );
    }
}
