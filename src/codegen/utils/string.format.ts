...
export const formatSolidityParameters = (props: IProperty[]): string =>
  props
    .map((prop) =>
      dynamicAllocTypes.some((el) => prop.type.includes(el)) ||
      prop.type == "bytes" ||
...
export const optionalComma = (props: IProperty[]) =>
  `${props?.length != 0 ? "," : ""}`;
export const optionalString = (props: IProperty[], delegate: any) =>
  `${props?.length != 0 ? delegate(props) : ""}`;
